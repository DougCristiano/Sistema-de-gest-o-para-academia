...
export const TeacherSchedule: React.FC = () => {
  ...
  const tomorrowAppointments = myAppointments.filter((apt) => isTomorrow(apt.date));
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">Visualize e gerencie suas aulas agendadas</p>
      </div>
      ...
      {/* Weekly Chart */}
      <Card className="p-6">
        <h2 className="text-xl font-bold mb-4">Distribuição Semanal de Aulas</h2>
      ...
      {/* Tabs */}
      <Tabs value={selectedDate} onValueChange={(value: any) => setSelectedDate(value)}>
        <TabsList>
          ...
          {appointments.length === 0 ? (
            <Card className="p-12">
              <div className="text-center">
                ...
                <p className="text-gray-600">Você não tem aulas agendadas para este período</p>
              </div>
            </Card>
          ) : (
            <div className="space-y-6">
              {/* Group appointments by date */}
              {Array.from(new Set(appointments.map((apt) => apt.date.toDateString()))).map(
                (dateString) => {
